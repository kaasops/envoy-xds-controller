...
func (s *LegacyStore) ReplaceNodeDomainsIndex(idx map[string]map[string]struct{}) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if idx == nil {
...
func (s *LegacyStore) GetNodeDomainsIndex() map[string]map[string]struct{} {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *LegacyStore) GetNodeDomainsForNodes(nodeIDs []string) (map[string]map[string]struct{}, []string) {
	s.mu.RLock()
	defer s.mu.RUnlock()
