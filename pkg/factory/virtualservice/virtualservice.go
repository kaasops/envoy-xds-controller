	...
	if f.Spec.AccessLogConfig != nil {
		...
		if val, ok := accessLogConfig.Annotations[options.AutoGeneratedFilename]; ok {
			...
			if val {
				...
				if !ok {
					return nil, errors.NewUKS(fmt.Sprintf("Annotation %s worked only for AccessLogConfig with file type", options.AutoGeneratedFilename))
				}

				if err := configType.TypedConfig.UnmarshalTo(fileConfig); err != nil {
					return nil, errors.WrapUKS(err, fmt.Sprintf("Annotation %s worked only for AccessLogConfig with file type", options.AutoGeneratedFilename))
				}
				fileConfig.Path += fmt.Sprintf("/%s.log", f.Name)
