...
import { Card, CardContent, Grid, IconButton, List, ListItemButton, Typography, useMediaQuery, useTheme } from "@mui/material";
...
const LocationCard: React.FC<{ domain: IDomainLocationsResponse }> = ({ domain }) => {
    ...
    const { data: listenerData, isFetching: listenerIsFetching } = useGetListenerApi(nodeID as string, domain.listener, loadListener);

    const { data: filterChainData, isFetching: filterChainIsFetching } = useGetFilterChain(nodeID as string, domain.listener, domain.filter_chain, loadFilterChain);

    const { data: filterNameData, isFetching: filterNameIsFetching } = useGetFilterName(nodeID as string, domain.listener, domain.filter_chain, domain.filter, loadFilterName);

    const { data: routeConfigurationsData, isFetching: routeConfigurationsIsFetching } = useGetRouteConfigurations(nodeID as string, domain.route_configuration, loadRouteConfig)
    ...
    useEffect(() => {
        ...
        listenerData, listenerIsFetching,
        filterChainData, filterChainIsFetching,
        filterNameData, filterNameIsFetching,
        routeConfigurationsData, routeConfigurationsIsFetching,
        modalConfigName, openModal
    ]);
    ...
    const renderLocationItems = locationKeys.map((key, index) => (
        <ListItemButton sx={styleListItemButton}
            ...
            <IconButton className='CardRowCopy' onClick={(event) => {
    ...
    return (
        <>
            ...
            <ModalEnvoyConfig open={openModal}
