...
function CodeBlock({ jsonData, yamlData, heighCodeBox }: any) {
    ...
    return (
        <Box className='CodeBox' display='flex' flexDirection='column' height={`${heighCodeBox}%`}>
            ...
            <Box display='flex' justifyContent='flex-end' margin={'16px 23px'} gap={2}>
                ...
                <Button variant='contained' onClick={handleDownLoadClick}>Download {tabIndex === 0 ? 'JSON' : 'YAML'}</Button>
            </Box>
        </Box>
    )
}
