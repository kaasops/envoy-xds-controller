...
function DomainSection() {
    ...
    const { data: domainLocations, isFetching: domainLocationsFetching } = useGetDomainLocations(nodeID as string, domain as string)
        ...
        <Box className='DomainSection' sx={{ ...styleDomainSection, backgroundColor: colors.primary[800] }}>
            {domains?.length !== 0 ? (
                <Grid container height='100%'>
                    ...
                    <Grid className='DomainLocationSection'
                        item xs={12} md={8.5} lg={9}
                        height={'100%'}
                    >
                        <Box padding={3} paddingLeft={0} height='100%'>
                            <DomainLocations locations={domainLocations as IDomainLocationsResponse[]}
