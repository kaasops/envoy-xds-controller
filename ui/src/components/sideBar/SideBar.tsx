...
function SideBar({ isSmallScreen }: ISideBarProps): JSX.Element {
    ...
    const renderNavMenu = navMenuItems.map(menuItem => (
        <ListItemButtonNav key={menuItem.id}
            onClick={() => navigate(menuItem.path)}
            className={activePage.includes(menuItem.path) ? 'active' : ''}
        >
            <ListItemIcon sx={{ color: colors.gray[400] }}>
                {menuItem.icon}
            </ListItemIcon>
                ...
                >
                    <Box className='drawerHeader' sx={{ borderBottom: `1px solid ${colors.primary[100]}` }}>
                        <DrawerHeader>
                            ...
                            {isOpenSideBar && (
                                <IconButton onClick={() => toggleSideBar(!isOpenSideBar)}
